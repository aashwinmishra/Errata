...
class UNET_ResidualBlock(nn.Module):
  def __init__(self, in_channels, out_channels, n_time=1280):
    super().__init__()
    self.groupnorm_fature = nn.GroupNorm(32, in_channels)
  ...
  def forward(self, feature, time): #feature=latent= [B, in_ch, H, W], time=[1, 1280]
    residue = feature 
    feature = self.groupnorm_fature(feature)
...
class UNET_AttentionBlock(nn.Module):
  ...
  def forward(self, x, context): #X=[B, Features, H, W], c=[B, S, Dim]
    residue_long = x
